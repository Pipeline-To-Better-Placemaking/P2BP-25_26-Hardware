...
# Automatically load homographies per camera name
...
def assign_global_id(cam_id, feat, now):
    global NEXT_GID
    ...
    with GID_LOCK:
        for gid, data in GLOBAL_IDENTITIES.items():
            if now - data["last_seen"] > MAX_IDLE_TIME:
                continue
            if data["count"] < 3:
                continue
            if cam_id in data["cams"] and now - data["last_seen"] > 5.0:
                continue
    ...
    def run(self):
        while True:
            ...
            for box, bt_id in zip(boxes, ids):
                ...
                if t is None:
                    t = self.tracks[bt_id] = {"sid": sid, "last": now, "last_feat": 0, "feats": []}
                t["last"] = now
                ...
                if self.frame_id - t["last_feat"] >= FEATURE_INTERVAL:
                    ...
                    gid = assign_global_id(self.cam_id, avg_feat, now)
                    TRACKS[sid]["gid"] = gid
