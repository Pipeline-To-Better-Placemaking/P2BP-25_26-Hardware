...
class CameraThread(threading.Thread):
    ...
    def run(self):
        ...
        while not self.stop_event.is_set():
            ...
            # DEBUG: print actual YOLO rate once per second
            now_fps = time.time()
            if now_fps - last_fps_report >= 1.0:
                actual_fps = yolo_frame_count / (now_fps - last_fps_report)
                print(f"[DEBUG] Camera {self.display_name}: YOLO+ByteTrack ≈ {actual_fps:.2f} Hz")
                yolo_frame_count = 0
