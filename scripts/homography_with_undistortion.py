...
NEW: Optionally uses ./config/camera_intrinsics.yml for lens undistortion before homography calculation.
...
def load_intrinsics(path: Path) -> Optional[Intrinsics]:
    """Load camera intrinsics from YAML file.
    
    Expected format (PyYAML style):
        camera_matrix: [[fx, 0, cx], [0, fy, cy], [0, 0, 1]]
        distortion_coefficients: [k1, k2, p1, p2, k3]
        image_width: 3072
        image_height: 1728
    
    Returns Intrinsics object or None if file not found/invalid.
    """
    if not path.exists():
        return None
    ...
    try:
        ...
        if not isinstance(data, dict):
            raise ValueError(f"Invalid intrinsics YAML format: {path}")

        camera_matrix = np.asarray(data["camera_matrix"], dtype=np.float64)
        distortion = np.asarray(data["distortion_coefficients"], dtype=np.float64).reshape(-1)
        ref_w = int(data["image_width"])
        ref_h = int(data["image_height"])

        if camera_matrix.shape != (3, 3):
            raise ValueError(f"Expected 3x3 camera_matrix, got {camera_matrix.shape}")
        if distortion.size < 4:
            raise ValueError(f"Expected >=4 distortion coeffs, got {distortion.size}")
        ...
        print(f"Warning: Could not load intrinsics from {path}: {e}")
        return None
...
def scale_camera_matrix(camera_matrix: np.ndarray, 
    ...
    """Scale camera matrix from source resolution to destination resolution.
    
    Args:
        camera_matrix: 3x3 intrinsic matrix calibrated at src_size
        src_size: (width, height) the intrinsics were calibrated at
        dst_size: (width, height) of the actual image
    
    Returns:
        Scaled 3x3 camera matrix
    """
...
class BoardSpec:
    ...
    squares_x: int
    squares_y: int
    square_length: float #world units
    marker_length: float
...
def compute_homography_from_frame(
    ...
    min_corners: int,
    intrinsics: Optional[Intrinsics] = None,  # NEW: single Intrinsics object
) -> Optional[HomographyResult]:
    h_img, w_img = frame_bgr.shape[:2]
    gray = cv2.cvtColor(frame_bgr, cv2.COLOR_BGR2GRAY)

    # Apply lens undistortion if intrinsics are provided
    if intrinsics is not None:
        # Scale camera matrix if intrinsics were calibrated at different resolution
        scaled_k = scale_camera_matrix(
            intrinsics.camera_matrix, 
            intrinsics.reference_size, 
            (w_img, h_img)
        )
        
        # Undistort the grayscale image before detection
        gray = cv2.undistort(gray, scaled_k, intrinsics.distortion)
...
def save_homography_yaml(
    ...
    p2_world: np.ndarray,
    ransac_thresh_px: float,
    undistortion_applied: bool = False,  # NEW
) -> None:
    ...
    try:
        ...
        fs.write("undistortion_applied", 1 if undistortion_applied else 0)  # NEW
...
def run_once(base_dir: Path) -> None:
    ...
    intrinsics_path = base_dir / "config" / "camera_intrinsics.yml"
    intrinsics = load_intrinsics(intrinsics_path)
    
    if intrinsics is not None:
        print(f"Loaded camera intrinsics from {intrinsics_path}")
        print(f"  Reference resolution: {intrinsics.reference_size[0]}x{intrinsics.reference_size[1]}")
        print(f"  Distortion coeffs: {intrinsics.distortion[:3]}...")
    else:
        print("No camera intrinsics found - skipping lens undistortion")
    ...
    for cam_key in cam_keys:
        ...
        for f in frames:
            try:
                res = compute_homography_from_frame(
                    ...
                    intrinsics=intrinsics,  # pass Intrinsics object
                )
            except Exception:
                res = None
        ...
        save_homography_yaml(
            ...
            p2_world=p2_world,
            ransac_thresh_px=ransac_thresh_px,
            undistortion_applied=(intrinsics is not None),  # NEW
        )

        print(f"[{cam_key}] saved: {out_path} (inliers={best.inliers}, rmse={best.rmse_world:.3f}, undistorted={intrinsics is not None})")
        any_success = True
